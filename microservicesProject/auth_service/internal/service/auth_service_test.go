...
func TestAuthService_Authenticate(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			mockRepo := mock_repository.NewMockAuthorizationRepos(ctrl)
			tc.mockBehavior(mockRepo, tc.args.UserEmail, tc.args.UserPassword, context.Background()) // Call mockBehavior

			authService := service.NewAuthService(mockRepo) // Ensure AuthService is created with the mock

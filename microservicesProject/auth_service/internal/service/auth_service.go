...
import (
	"context"
	"fmt"
	"microservicesProject/auth_service/internal/erro"
...
func (as *AuthService) Registrate(user *model.Person, ctx context.Context) *AuthenticationServiceResponse {
	...
	if err != nil {
		hashmapa := make(map[string]interface{})
		hashmapa["HashPass"] = erro.ErrorHashPass
		return &AuthenticationServiceResponse{Success: false, Errors: err}
	}
	...
	if !response.Success {
		return &AuthenticationServiceResponse{Success: false, Errors: response.Errors}
	}
	return &AuthenticationServiceResponse{Success: true, UserId: response.UserId}
}
func (as *AuthService) Authorizate(user *model.Person, ctx context.Context) *AuthenticationServiceResponse {
...
func validatePerson(as *AuthService, user *model.Person, flag bool) map[string]interface{} {
...
func (as *AuthService) CheckSession(sessionID string) *AuthenticationServiceResponse {

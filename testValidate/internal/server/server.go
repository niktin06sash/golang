...
func (server *Server) Handlers() http.Handler {
	mu := mux.NewRouter()
	mu.HandleFunc("/reg", server.NonAuthorityMiddleware(http.HandlerFunc(server.Registration)).ServeHTTP).Methods("POST")
	mu.HandleFunc("/auth", server.NonAuthorityMiddleware(http.HandlerFunc(server.Authentication)).ServeHTTP).Methods("POST")
	mu.HandleFunc("/logout", server.AuthorityMiddleware(http.HandlerFunc(server.LogoutHandler)).ServeHTTP).Methods("POST")
	mu.HandleFunc("/", server.NonAuthorityMiddleware(http.HandlerFunc(server.MainPage)).ServeHTTP).Methods("GET")
	mu.HandleFunc("/profile", server.AuthorityMiddleware(http.HandlerFunc(server.PersonPage)).ServeHTTP).Methods("GET")
	mu.HandleFunc("/greeting", server.AuthorityMiddleware(http.HandlerFunc(server.GreetingPage)).ServeHTTP).Methods("GET")
	return mu
}
